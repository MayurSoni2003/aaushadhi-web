...
function buildWhatsAppUrl(product: CartProduct, qty: number) {
  const totalGrams = qty * 100;
  const lineTotal = product.price * qty;
  const message = [
    "*Aaushadhi Wellness — New Order*",
    "",
    `1. ${product.productName} × ${qty} (${totalGrams}g) — ₹${lineTotal}`,
    "",
    `*Total: ₹${lineTotal}*`,
    "",
      ...
      {qty === 0 ? (
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button
            type="button"
            onClick={() => {
              window.open(buildWhatsAppUrl(product, 1), "_blank");
            }}
            className="
              w-full sm:w-auto px-12 py-4 rounded-full text-sm font-bold uppercase tracking-wider
              bg-[#25D366] text-white flex items-center justify-center gap-2.5
              hover:bg-[#20BD5A]
              active:scale-[0.98]
              transition-all duration-200
              cursor-pointer
        ...
        <div className="flex flex-col gap-4">
          ...
          <button
            type="button"
            onClick={() => {
              window.open(buildWhatsAppUrl(product, qty), "_blank");
            }}
            className="
